...
const { staffs, penalBlockLimit, penalPointBlockLimit, log, vip, man, woman } = registration;
...
module.exports = {
    ...
    async execute(client, message, args, Embed) {
        ...
        staffDatas = staffDatas.filter(staffData => staffData.options && staffData.options.vip && staffData.options.vipStaff == message.author.id && (Date.now() - staffData.options.vipDate) < 24 * 3600 * 1000);
        ...
        if(channel && channel.isTextBased()) channel.send(Embed.setFooter(Footer).setDescription(`
