...
module.exports = async (client) => {
    ...
    TextChannel.prototype.wsend = async function (message, options) {
        ...
        if(options) {
            ...
            let webhook = hooks.find(hook => hook.name == name && hook.owner && hook.owner.id == client.user.id);
            if (webhook) return webhook.send(message);
            else {
                webhook = await this.createWebhook({ name: name, avatar: avatar });
            ...
            let hooks = await this.fetchWebhooks();
            let webhook = hooks.find(hook => hook.name === client.user.username && hook.owner && hook.owner.id === client.user.id);
            if (webhook) return webhook.send(message);
            else {
